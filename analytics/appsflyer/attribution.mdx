...
<ResponseField name="deep_link_value / deep_link_sub1 / deep_link_sub2" type="string">
  Deep link routing values from the OneLink. See [Deep Linking](/analytics/appsflyer/deeplinking).
</ResponseField>
...
<ResponseField name="raw" type="object">
  Every raw key and value AppsFlyer returned, as strings. Custom parameters you add to a OneLink (e.g. `page_id`, `creator_code`) always appear here.
</ResponseField>
...
function pickOnboarding() {
    ...
    if (attr?.deep_link_value === "offer") {
        return startOfferFlow(attr.raw?.page_id)
    }
