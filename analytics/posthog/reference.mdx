...
The PostHog bridge exposes schemes for capturing events, identifying users, managing groups, tracking screens, and controlling GDPR consent. Each one is fire-and-forget. Nothing to await, nothing to read back from the call - state changes surface through the [injected globals](/analytics/posthog/introduction) that the native layer keeps live.
...
- All other string params - event names, `distinct_id`, group keys, screen names, super property keys - are passed as-is.
