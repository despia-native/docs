...
<Update label="May 17, 2026" description="File viewer scheme - native preview for PDFs, images, and Office docs">
  ## Native file preview with `fileviewer://`

  A new `fileviewer://` URL scheme opens any remote file in the native OS previewer, using QuickLook on iOS and the system "Open with" chooser on Android. Files served behind your existing login work without extra token plumbing, since the download reuses the WebView's current cookies and User-Agent.

  **What ships today:**

  - **Single cross-platform call.** `despia('fileviewer://?src=<url>')` works identically on iOS and Android, with the same encoding rules and the same fire-and-forget contract.
