...
<Update label="May 16, 2026" description="File viewer scheme - native preview with custom Android engine">
  ...
  **What ships today:**
  ...
  - **Custom Android previewer.** Android uses Despia's own in-app file preview engine, not the system "Open with" chooser. This removes the dependency on whichever third-party file viewer the user happens to have installed and delivers render fidelity on par with QuickLook on iOS. Comparable commercial Android file viewer SDKs are typically licensed at \$1,500 or more per project, and this is included in Despia at no additional cost.
  ...
  - **Same interception model as other in-app schemes.** Matches the navigation-hook pattern used by `printitem://` and `savethisimage://`, so no web-side setup is required beyond calling the scheme.
