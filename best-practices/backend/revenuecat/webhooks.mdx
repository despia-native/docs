...
| :---------------------------- | :---------- | :---------- |
| After processing webhook      | ✅ Yes       | ⚠️ Optional |
| Real-time status check        | ❌ No        | ✅ Yes       |
| Self-healing validation       | ❌ No        | ✅ Yes       |
| Getting subscriber attributes | ✅ Yes       | ❌ No        |
| Checking active entitlements  | ⚠️ Possible | ✅ Better    |
| Historical purchase data      | ✅ Yes       | ⚠️ Limited  |
...
```mermaid
flowchart TD
    RC[RevenueCat - Source of Truth<br/>• App Store / Play Store / Stripe<br/>• Handles refunds, renewals, cancellations]
    ...
    subgraph DB_Tables [" "]
        T1[user_subscriptions<br/>• is_active<br/>• entitlements JSON<br/>• expires_at<br/>• product_id]
        T2[user_entitlements<br/>• entitlement_id<br/>• expires_at<br/>• is_active<br/>• product_id]
    end
    
    DB -.-> DB_Tables
    
    DB -->|API V2 - Backup Verification<br/>When:<br/>• Webhook missed<br/>• Near expiration<br/>• Billing issues<br/>• User-triggered check| APP
    
    APP[Your Application<br/>• Feature gates<br/>• Access control<br/>• UI rendering]
