...
1. **Vault fails/empty on startup** → Create new `app_user_id` from `despia.uuid`, try to save to vault
2. \*\*User logs in with account that has no \*\*`app_user_id` → CLAIM: assign current ID to account
3. \*\*User logs in with account that has existing \*\*`app_user_id` → RECOVER: use account's ID, heal vault
4. **Always write to vault after login** → Ensures vault is synced with account's correct ID
5. **Vault write fails** → Continue anyway, login will heal it next time
...
## RevenueCat → Push Notification Flow
...
## Webhook Event → Push Notification
...
flowchart TD
    ...
    Switch -->|INITIAL_PURCHASE| Purchase["🎉 Welcome to Premium!<br/>Thank you for subscribing"]
    Switch -->|TRIAL_STARTED| Trial["🎉 Trial Started!<br/>Explore premium features"]
    Switch -->|BILLING_ISSUE| Billing["⚠️ Payment Issue<br/>Update payment method"]
    Switch -->|CANCELLATION| Cancel["Before you go...<br/>Tell us what went wrong"]
    Switch -->|EXPIRATION| Expire["We miss you! 👋<br/>Come back for 20% off"]
    Switch -->|RENEWAL| Renew["✅ Subscription Renewed<br/>Thanks for staying"]
  ...
  switch (eventType) {
    ...
    case 'INITIAL_PURCHASE':
      await sendToOneSignal({
        ...
        headings: { en: '🎉 Welcome to Premium!' },
    ...
    case 'RENEWAL':
      await sendToOneSignal({
        app_id: ONESIGNAL_APP_ID,
        include_external_user_ids: [appUserId],
        headings: { en: '✅ Subscription Renewed' },
