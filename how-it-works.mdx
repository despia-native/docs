---
title: "Introduction"
description: "Technical deep dive into offline media caching with background downloads"
---

Local CDN solves offline media caching for hybrid apps - a problem that has challenged web developers for over 15 years. Downloads continue in the background even when users close the app.

<Info>
  Local CDN uses native background task APIs (NSURLSession on iOS, WorkManager on Android) with built-in retry mechanisms. Start a download, close the app, get notified when it's ready. When users reopen your app, `window.contentServerChange(item)` is called for each item that completed in the background.
</Info>
...
<Card icon="book" href="/local-server" title="Learn More">
...
When you call `localcdn://write`, here's exactly what happens:
...
// It's called once per download - both foreground completions AND background replays
window.contentServerChange = (item) => {
  console.log("Content ready:", item.index);
  console.log("Local URL:", item.local_cdn);
  
  // Add to your downloads list
  addToDownloadsList(item);
...
Just implement `contentServerChange` and it will be called for both:
...
window.contentServerChange = (item) => {
  ...
  if (existingIndex >= 0) {
    ...
    // This is a new download - show notification
...
This is the Netflix/Spotify experience: start downloads, forget about them, get notified when ready.
...
- `localcdn://write` \> `/localcdn/` directory
...
<CardGroup cols={2}>
  <Card icon="video" title="Media-heavy Applications">
  ...
  <Card icon="wifi-slash" title="Offline-first Experiences">
  ...
  <Card icon="upload" title="User-generated Content">
  ...
  <Card icon="bolt" title="Performance-critical Applications">
...
<AccordionGroup>
  <Accordion defaultOpen icon="sparkles" title="How much storage can I use?">
    The device's available storage. No artificial quotas like IndexedDB. Cache gigabytes of content.
  </Accordion>
  <Accordion defaultOpen icon="sparkles" title="Does this work offline?">
  ...
  <Accordion defaultOpen title="What file types are supported?">
    Any file type: video (mp4, webm, mov), audio (mp3, wav, aac), images (jpg, png, webp), documents (pdf, json), and more.
  </Accordion>
  <Accordion defaultOpen title="Is this App Store compliant?">
  ...
  <Accordion title="How do I know if a download succeeded?">
    ...
    **Note:** Do not await the write call with a key - the JS bridge has a ~30s timeout that will resolve with `null` for large files, even though the download continues.

    **Optional:** If you need more control, poll via `localcdn://read` to check status.
  </Accordion>
  ...
  <Accordion title="Can I upload files from a file picker?">
    Yes, but only when your app is served via Despia Local Server (not from origin). POST to `http://localhost:7777/api/upload` with FormData. Works with any File/Blob source - file inputs, camera, clipboard, etc. Uploaded files are stored in `/files/` directory.
  </Accordion>
  ...
  <Accordion title="How do I track which downloads completed in the background?">
    You don't need to track this manually. When the app reopens after background downloads, the native layer automatically calls `contentServerChange(item)` for each item that completed while backgrounded. Just implement `contentServerChange` and it handles both foreground completions and background replays.
  </Accordion>
...
<CardGroup cols={3}>
  <Card icon="memory" title="O(1) Memory">
    Fixed memory usage regardless of file size
  </Card>
  <Card icon="hard-drive" title="Device Storage">
    Use full device capacity, no artificial quotas
  </Card>
  <Card icon="wifi-slash" title="Complete Offline">
    Works without any network connectivity
  </Card>
  <Card icon="microchip" title="Hardware Accelerated">
    Native playback performance
  </Card>
  <Card icon="code" title="Unified SDK">
    Same API for iOS and Android
  </Card>
  <Card icon="cloud" title="No Cloud Lock-in">
...
<CardGroup cols={2}>
  ...
  <Card icon="code" href="/local-cdn/reference" title="Local CDN API Reference">
    Quick start guide
  </Card>
  <Card icon="server" href="/local-server" title="Despia Local Server">
