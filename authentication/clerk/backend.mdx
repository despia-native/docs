...
`clerk://ssr` writes the session into the WebView's cookie store (and optionally injects a request header) so your page's server middleware sees an authenticated user on the very first request, no Clerk web SDK on the page required. On by default with sensible scoping, so most apps never call this route directly.
