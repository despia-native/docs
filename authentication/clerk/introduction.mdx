...
Wire the listener first, then call `clerk://configure`. Events are not buffered, a handler attached after a command was issued will miss the result.
...
Call `clerk://configure?key=pk_...` on every page load. Same-key calls re-emit `ready` as a no-op. Different keys switch tenant and clear the secure session store.
...
<CardGroup cols={2}>
  <Card title="Auth Methods" icon="right-to-bracket" href="/authentication/clerk/methods">
    Auth Sheet, email & password, OAuth, Apple, email code, phone code, passkeys.
  </Card>
