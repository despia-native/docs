...
After [sign-in](/authentication/clerk/methods), the bridge keeps `window.clerkJWT` populated and maintains an in-memory user snapshot you can read offline.

---

## Session State

`clerk://state` returns a full snapshot from the in-memory cache hydrated from the secure session store. No network round-trip. Calling it on a signed-in user also restarts the JWT refresher.
...
```json
{ "ok": true, "event": "state", "isConfigured": true, "isSignedIn": false, "publishableKey": "pk_test_abc123" }
```

When not configured:

```json
{ "ok": true, "event": "state", "isConfigured": false, "isSignedIn": false }
```
...
Refresher starts on configure, reconfigure, `clerk://state` while signed in, and any sign-in flow reaching `complete`. Stops on sign-out and reconfigure.

---

## Force a refresh

`clerk://token` forces a refresh and emits the result. Use this only when you need a guaranteed-fresh token right now.
...
```json
{ "ok": true, "event": "token", "jwt": "eyJhbGciOiJSUzI1Ni..." }
```
...
For middleware-based auth on Next.js, TanStack, and other frameworks, see [Server-Side Auth](/authentication/clerk/server-side).
