...
<Card title="AI Prompt" icon="sparkles">
  Add Sign in with TikTok to my app using the Despia SDK from: [https://www.npmjs.com/package/despia-native](https://www.npmjs.com/package/despia-native)
  ...
  TikTok uses authorization code flow. Tokens never arrive in the URL hash. Your backend exchanges the `code` for tokens using your TikTok Client Secret, which must never be exposed client-side.
