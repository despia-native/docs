...
| Cloud sync | Yes, sync resumes later | Yes | `db.powersync.connect`, `db.powersync.sync`, `db.powersync.status`, `db.powersync.events.status`, `db.powersync.events.upload` |
...
If your backend upload runs in JavaScript, register an upload handler so native can hand local CRUD writes to your HTTP layer:
...
The handler is called when native PowerSync has pending CRUD rows to send to your backend. If the handler resolves, the native queue is finalized. If it throws, PowerSync retries later. Register one upload handler per session.

---

## Search

Register native full-text indexes on any local SQLite table and query them with plain, prefix, or fuzzy matching:
...
<CardGroup cols={2}>
  ...
  <Card title="Reference" icon="code" href="/local-database/reference">
    Full API, init, query, execute, batch, watch, migrate, search, sync
  </Card>
