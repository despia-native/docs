---
title: "Reference"
description: "Full API reference for @despia/powersync, init, query, write, watch, search, sync, and migrate."
icon: "code"
---
...
Query a local SQLite database, run migrations, watch live results, search with full-text indexes, and optionally sync with your backend in real time.
...
Register a handler that native calls when it has local CRUD writes to send to your backend. Use this when your backend upload runs in JavaScript rather than natively.
...
The callback receives the pending CRUD batch and may return a promise. If the handler resolves, the SDK tells native `upload_complete` so the native queue can be finalized. If the handler throws, the SDK reports the error back to native and PowerSync retries later. Register one upload handler per app session — calling `events.upload()` again replaces the previous handler.

---

## Search

Search lets you register native full-text indexes on local SQLite tables and query them with plain, prefix, or fuzzy matching. Native uses SQLite FTS5 for full-text and prefix search, and native trigram/edit-distance ranking for `mode: "fuzzy"` when supported by the app build.

The SDK trims search names, table names, column names, and query text before sending them to native. Search index columns must be unique, query text is capped at `256` characters, and `limit` is capped at `1000` per request.

### db.search.index

Create or update a native full-text search index on a table.
...
Native validates that the source table and columns exist before creating the index.

<ParamField path="name" type="string" required>
  Unique name for this search index.
</ParamField>

<ParamField path="table" type="string" required>
  The SQLite table to index.
</ParamField>

<ParamField path="columns" type="string[]" required>
  The columns to include in the full-text index. Must be non-empty and reference columns that exist in the table.
</ParamField>
...
List all native search indexes currently registered for the database.
...
<ParamField path="text" type="string" required>
  Search text. Capped at 256 characters.
</ParamField>

<ParamField path="mode" type='"plain" | "prefix" | "fuzzy"' required>
  Search mode. `"plain"` matches exact terms, `"prefix"` matches term prefixes, `"fuzzy"` uses typo-tolerant ranking when supported by the app build.
</ParamField>
...
### db.search.rebuildIndex

Rebuild a search index. Use this after large migrations or data repairs if your native integration uses rebuild-based fuzzy indexes.
...
Remove a search index permanently.
...
export type PowerSyncCrudEntry = {
    id:        string
    table:     string
...
export type PowerSyncUploadPayload = {
    crud: PowerSyncCrudEntry[]
}

export type PowerSyncUploadHandler = (
    payload: PowerSyncUploadPayload
) => void | Promise<void>

export type PowerSyncSearchMode = 'plain' | 'prefix' | 'fuzzy'

export type PowerSyncSearchIndexOptions = {
    name:    string
    table:   string
    columns: string[]
}

export type PowerSyncSearchIndex = {
    name:    string
    table:   string
    columns: string[]
}

export type PowerSyncSearchQueryOptions = {
    index: string
    text:  string
    mode:  PowerSyncSearchMode
    limit?: number
}

export type PowerSyncSearchRow<T> = T & {
    _rank?: number
}

export type PowerSyncSearchDropIndexOptions = {
    name: string
}

export type PowerSyncSearchRebuildIndexOptions = {
    name: string
}
