...
Your PowerSync App ID is configured in the Despia dashboard, not in code. This keeps credentials out of your JavaScript bundle and lets you change instances without shipping a new build.

<Steps>
  <Step title="Open the Integrations tab">
    Go to **Despia** → **App** → **Integrations**.
  </Step>
  <Step title="Paste your PowerSync App ID">
    Under the **PowerSync** section, paste your App ID into the **App ID** field and save.
  </Step>
</Steps>

Once saved, the native bridge routes `db.connect()` calls to the configured PowerSync instance automatically.
...
Establish a connection to your PowerSync instance. `fetchToken` is called whenever a new JWT is needed. Pass a `schema` object so the sync engine knows which tables and columns to map from your backend.

The PowerSync App ID is read from the Despia dashboard configuration, so you do not pass a URL in code.
...
```mermaid
flowchart TD
    A["App ID saved in Despia dashboard"] --> B["db.connect({ fetchToken, schema })"]
    B --> C["JWT fetched from your backend"]
    C --> D["Native bridge connects to PowerSync using dashboard App ID"]
    D --> E{"App State?"}
    E -->|"Online"| F["Data syncs in background"]
    E -->|"Offline"| G["Reads and writes hit local SQLite"]
    G --> H["Pending writes queued"]
    H --> I["Device comes online"]
    I --> F
    F --> J["watch() callbacks fire with updated rows"]
```
